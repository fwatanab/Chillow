...
- `/admin/events` のサーバー送信イベント (SSE) で通報・BAN の更新をリアルタイム受信し、手動リロードなしで反映される。

## 6. UI / レイアウト概要

- ダークテーマ（背景 `#2f3136`）をベースに、サイドバー/ヘッダー/フッターはわずかに濃い `#292b31` を採用。PC/モバイル共通で統一感を持たせ、境界線 (`border-gray-800`) で区切りを明確化。
- サイドバーのフレンド一覧はカード調で表示し、1ユーザーごとに余白・枠線を与えて視認性を確保。モバイルでは同じコンポーネントをフルスクリーンで利用。
- チャットルームは PC ではサイドバー＋メイン、モバイルでは「サイドバー＝ホーム」「戻る付きチャット」の 2 画面構成。ステータスバッジ（オンライン緑ドット）と「写真/ファイル」切替 `＋` ボタンを提供。
- スタンプ／絵文字ピッカーはタブで切替える統合 UI を採用し、プリセットは `frontend/src/constants/chatPalette.ts` から一元管理。

より詳しい色・レイアウトガイドは `docs/ui-style-guide.md` を参照。
